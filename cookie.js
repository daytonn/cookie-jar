(function(w) {
  ...
  w.Cookie.prototype.to_string = function() {
    return [
  ...
  w.Cookie.prototype.get = function() {
    var pairs = getPairs();
    return pairs[this.name];
  };

  w.Cookie.prototype.set = function() {
    document.cookie = this.to_string();
  };

  w.Cookie.prototype.remove = function() {
    this.value = '';
    this.exp = createExpiryDate(-1);
    this.set();
  };

  w.CookieJar = function(options) {
    var settings = {
      		path: '/',
      		daysToLive: 30,
      		autoSet: false
  ...
  w.CookieJar.prototype.get = function(cookie) {
    if (cookie === undefined) {
  ...
  w.CookieJar.prototype.set = function(cookie, value) {
    if (cookie === undefined) {
  ...
  w.CookieJar.prototype.remove = function(cookie) {
    if (cookie === undefined) {
  ...
  w.CookieJar.prototype.add = function(cookie, value) {
    if (cookie === undefined) {
